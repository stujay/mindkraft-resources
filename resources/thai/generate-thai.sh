...
OUTPUT_FILE="$OUTPUT_DIR/thai_brahmi_updated.json"

# Create the output directory if it doesn't exist
mkdir -p "$OUTPUT_DIR"

# Use jq to update the brahmi_parent field based on the symbol
jq '
  .consonants |= map(
    .brahmi_parent = 
      if .symbol == "ก" then "ka"
...
echo "Updated thai.json with brahmi_parent field changed has been exported to $OUTPUT_FILE"
